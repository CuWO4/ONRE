...
/* === interface === */
#define USE_ONRE \
using onre::operator""_match; \
using onre::operator""_replace
...
template<char... Cs>
constexpr auto build_fx_str() {
  constexpr char arr[] = { Cs..., '\0' };
  return impl::FixedString<sizeof...(Cs) + 1>(arr);
}
  ...
  // gcc do not trigger the warning.
#else
  static_assert(false, "This version of library does not support compilers other than clang and gcc. Please use the main branch that supports standard C++.");
#endif

// GNU extension feature, suppress warning to stop compiler complaining

template<typename CharType, CharType... Cs>
constexpr auto operator""_match() {
  static_assert(std::is_same_v<CharType, char>, "only permit char string");
  return Matcher<build_fx_str<Cs...>()> {};
}

template<typename CharType, CharType... Cs>
constexpr auto operator""_replace() {
  static_assert(std::is_same_v<CharType, char>, "only permit char string");
  return Replacer<build_fx_str<Cs...>()> {};
}
