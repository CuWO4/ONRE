# 本仓库已转入私有仓库, 并添加大量颠覆性功能, 待稳定后会重新发布. 该库仅为历史版本.
...
单头文件的零成本抽象的正则引擎, 对待匹配串长度具有严格线性时间复杂度.

---

## ✨ 核心特征

- ✔️ 严格 $O(|s|)$ 的匹配复杂度 (对固定 Pattern, 常数与编译出的自动机规模有关; 涉及捕获组的操作常数更大), $|s|$ 为待匹配串长度;
- ✔️ 在编译期将常量字符串正则表达式构建为自动机, 无运行时开销;
- ✔️ 单头文件, 仅需 `include` 即可使用;
- ✔️ **支持捕获组**以及基于捕获组的替换;
- ✔️ 使用 Brzozowski 导数进行基于类型体操的函数式元编程, 确保编译迅速且最终结果几乎总是最简自动机, 而且很酷!
- ✔️ 完整的 UTF-8 支持, 包括按码点匹配的通配符 `.` 和按码点工作的字符类/范围, 但反向字符类不支持 UTF-8;
- ✔️ 支持标准正则表达式(连接, 并 `|`, 闭包 `*`, 括号 `()`); 支持 `+`, `?`; 支持通配 `.`; 支持字符类 (形如 `[^a-z012]` 和 `[^一-上]`); 支持字符转义 (`\n`, `\t`, `\d`, `\s`, `\w`, `\xHH` (两位十六进制), `\u{H...}` (Unicode 码点), `\[`, `\]`, `\*`...); 支持非捕获组 `(?:...)`; 支持量词 (`{n}`, `{n,}`, `{,m}`, `{n,m}`); 支持注释 `(?#...)`; 默认情况下 `.` 不匹配 `\n` 或 `\r`, 如需让 `.` 也匹配它们, 可在编译时定义 `ONRE_DOTALL`;
- ❌️ 不支持零宽断言;
- ❌️ 不支持反向引用;
...
警告: 尽管动态运行时间是严格有上界的, 编译时间总体上是可采纳的, 但是严重的回溯地狱仍然可能显著拖慢编译速度甚至使编译器崩溃. 例如 `([^a]*)([^b]*)([^c]*)([^d]*)([^e]*)` 这个模式在我的电脑上花费了 1min18s 进行编译. 该库只在保证运行时间优化最大化的情况下, 尽量优化静态编译性能, 因此不要写过于明显可避免的回溯地狱, 以拖慢编译速度.
...
UTF-8 字符类和范围是按码点工作的, 反向字符类也同样支持 UTF-8。非法 UTF-8 输入不会匹配。
...
若 `onre::replace` 无法匹配, `onre::replace` 返回空串 (与替换规则不合法时相同). 对于有可能匹配失败的场景, 或需要区分失败原因的场景, 应该先使用 `onre::match` 检查是否匹配.
...
例如对于 POSIX 正则表达式 `a(a*)a`, 它尝试捕获去除首字母和尾字母的串, 它可以看作扩展正则表达式 $\langle 0\rangle a^\ast\langle 1\rangle a$, 那么在完成匹配时, 将给出输出 $(s_0, s_1)$, 即为捕获组在待匹配串的起止索引(从 0 开始, 左闭右开).
...
我们再考虑仲裁函数的性质. 根据前文我们知道, 即使仲裁函数永远选第一个或者第二个, 或者随机选择, 算法最后也会给出可能的槽格局. 那么当可能槽格局数为1时(原表达式无歧义), 给出的输出总能正确匹配捕获组; 对于有歧义的情况, 由于 TNFA 和原表达式高度异构, 我们难以通过仲裁函数使得 TNFA 输出完全兼容消除歧义标准(例如贪心最长). 但我们可以通过一系列启发式指标和算法来尽可能匹配最长.
