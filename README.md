...
- ✔️ 支持标准正则表达式(即连接, 并, 闭包, 括号); **支持捕获组**; 支持 `+`, `?`; 支持通配 `.`; 支持字符类 (形如 `[^a-z012]`); 支持字符转义 (`\n`, `\t`, `\f`, `\r`, `\d`, `\s`, `\w`, `\x[HEX]`, `\\`, `\[`, `\]`, `\(`, `\)`, `\*`...); 支持量词 (`{n}`, `{n,}`, `{,m}`, `{n,m}`);
- ❌️ 不支持零宽断言; 不支持反向引用; 捕获规则不保证符合 POSIX 标准.
...
❗ `onre::Replace<((a*)(a*)b)*>::replace("$2:$3", "aaabaab")` 会错误地得到 `"aaa:a"` 而非预期的 `aaa:`. 只要是闭包内部存在最长匹配竞争的表达式, 并且外部闭包重复超过一次时就会触发, 但只要闭包内不存在竞争最长匹配的表达式, 或外部闭包只重复了一次, 就不会出现该错误.

🩹 以上问题涉及的表达式几乎不会在正常使用中出现, 并且由于模式字符串是静态的, 所以也不会被外部利用攻击.
